...
public class EventLoggerConsole extends MessageConsole {
	...
	public void write(String data, int type) {
		...
		if (type != Type_Error && type != Type_DebugInfo
				&& !EHActivator.getDefault().getPreferenceStore().getBoolean(Initializer.Pref_WriteToConsole)) {
			return;
		}

		if (EHActivator.getDefault().getPreferenceStore().getBoolean(Initializer.Pref_ShowConsole)) {
			try {
