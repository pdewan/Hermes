...
/*			"pasteTimes","numeric",
			"copyTimes","numeric",
			"longestDelete","numeric",
			"longestInsert","numeric",
...
//			System.out.println("Last prediction changed: " + newCommand + " ; index: " + participantTimeLine.getTimeStampList().size());
...
//		if (participantTimeLine.getTimeStampList().size() == 0) return;// no previous time stamp
//
//		System.out.println("Prediction: " + lastPrediction +  "; Prediction added at: " + 
//		participantTimeLine.getPredictions().size() + "; Correction: " + lastCorrection + 
//		"; Correction added at: " + participantTimeLine.getPredictionCorrections().size());
//		
//		participantTimeLine.getPredictions().add(lastPrediction); // do not reset as it is not a status
...
//		System.out.println("newCommand current Thread: " + Thread.currentThread().getName() );
		if(!Thread.currentThread().getName().equals(DifficultyPredictionRunnable.DIFFICULTY_PREDICTION_THREAD_NAME)) return;
...
//		System.out.println("timestamp: " + new Date(aTimeStamp) + " ; relativeTime: " + newCommand.getTimestamp() + " ; index: " + anIndex + " ; commandCounts: " + commandCounts[anIndex] + " ; " + newCommand);
		...
		if (anIndex != -1) {
			...
			case DELETE:
				...
				((NilsParticipantTimeLine)participantTimeLine).getLongestDeleteList().set(anIndex, Math.max(listValue, ((Delete)newCommand).getLength()));
				break;
			case INSERT:
				listValue = ((NilsParticipantTimeLine)participantTimeLine).getLongestInsertList().get(anIndex);
				((NilsParticipantTimeLine)participantTimeLine).getLongestInsertList().set(anIndex, Math.max(listValue, ((Insert)newCommand).getLength()));
				break;
			case COMPILE:
...
//		System.out.println("newRatios current Thread: " + Thread.currentThread().getName() );
		if(!Thread.currentThread().getName().equals(DifficultyPredictionRunnable.DIFFICULTY_PREDICTION_THREAD_NAME)) return;
...
//		System.out.println("currentTime: " + new Date(currentTime) + " ; savedTimeStamp: " + newVal.getSavedTimeStamp());
	}
...
//							Double.toString(p.getTimeStampList().get(i)),
							...
							Double.toString(p.getRemoveList().get(i)) // actually delete
//							Double.toString(p.getWebLinks()==null?0:p.getWebLinks().get(i)==null?0:p.getWebLinks().get(i).size()),
							...
							Double.toString(((NilsParticipantTimeLine)p).getLongestDeleteList().get(i)),
							Double.toString(((NilsParticipantTimeLine)p).getLongestInsertList().get(i))
//							Double.toString(((NilsParticipantTimeLine)p).getCompileList().get(i))
 
							*/
					);
			
		}
		System.out.println("Last timestamp: " + (p.getTimeStampList().get(p.getTimeStampList().size() - 1) - p.getTimeStampList().get(0)) + "; number of commands: " + p.getDebugList().size() * 40);
	}
...
//		DifficultyRobot.getInstance().getEventAggregator().setEventAggregationStrategy(new AWindowAggregator());
		
//		Mediator mediator = DifficultyRobot.getInstance();
//		RatioBasedFeatureExtractor featureExtractor = new ANilsFeatureExtractor(mediator);
//		FeatureExtractionStrategy featureExtractionStrategy = new ANilsFeatureExtractionStrategy();
