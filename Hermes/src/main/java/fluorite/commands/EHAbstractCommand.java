...
public abstract class EHAbstractCommand /*extends AbstractCommand(*/ implements
		fluorite.commands.EHICommand {

	private static boolean incrementCommandID = true;
	private static int currentCommandID = -1;

	private static Set<EHICommandIndexListener> commandIndexListeners = new HashSet<EHICommandIndexListener>();

	public static void addCommandIndexListener(EHICommandIndexListener listener) {
		if (!commandIndexListeners.contains(listener)) {
			commandIndexListeners.add(listener);
		}
	}

	public static void removeCommandIndexListener(EHICommandIndexListener listener) {
		if (commandIndexListeners.contains(listener)) {
	...
	private static void fireCommandIndexChanged() {
		for (EHICommandIndexListener listener : commandIndexListeners) {
...
/// Stuff thathas been added	
	...
	public static boolean combineWith(EHAbstractCommand aCommand, EHICommand anotherCommand) {
//		IPreferenceStore prefStore = fluorite.plugin.Activator
//				.getDefault().getPreferenceStore();
